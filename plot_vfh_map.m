...
% *************************************************************************
%
% PLOT_VFH_MAP computes the vector heterogeneity for each clique and
% represents it in a map.
%
%   vfh_map = PLOT_VFH_MAP(angle,n_clique_1d)
%
%   Parameters:
...
%
%   Returns:
%       vfh_map (Double): Each element (i,j) is the vfh of that clique.

function vfh_map = plot_vfh_map(angle,n_clique_1d)
    ...
    vfh_map = [];
    for i = n_clique_1d:-1:1
            vfh_map = [vfh_map; score_matrix(i,:)];
    end
    ...
    colormap(cool)
    colorbar
    caxis([0 1]) % Adjust this for visualization
    title('Heterogeneity Map','FontSize',16)
    foot = sprintf('Score: %.3f',vfh);
    xlabel(foot);
    
    for row = 1:n_clique_1d
        for col = 1:n_clique_1d
            label = num2str(vfh_map(row,col),'%.2f');
            text(col,row,label,'HorizontalAlignment','center','FontSize',12,'FontWeight','bold','Color','k')
        end
    end
    ax=gca;
    set(gca,'XTickLabel',[])
